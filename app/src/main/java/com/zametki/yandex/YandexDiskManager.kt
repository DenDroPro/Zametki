...
object YandexDiskManager {
    ...
    /**
     * Ensure backup folder exists on Yandex Disk
     */
    suspend fun ensureFolder(token: String): Boolean = withContext(Dispatchers.IO) {
        try {
            val url = URL("https://cloud-api.yandex.net/v1/disk/resources?path=${Uri.encode(BACKUP_FOLDER)}")
